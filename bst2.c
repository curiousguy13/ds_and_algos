...
int max(int a,int b);
void inorder_height(node* root);
void getmax_depth(node* root,int *max);
...
int getheight(node* N)
{
    ...
    if(N==NULL)
        return 0;
    height=max(getheight(N->leftchild),getheight(N->rightchild))+1;
    return height;

}
//this function gives the values of height of all the nodes of tree
void inorder_height(node* root)
{
    if(root!=NULL)
...
void getmax_depth(node* root,int *max)
{
    if(root!=NULL)
    {
        getmax_depth(root->leftchild,max);
        if(root->depth > *max)
            *max=root->depth;
        getmax_depth(root->rightchild,max);
    }
    return;
}
int maxDepth(node* node)
...
void print_tree(node* root)
{
    ...
    do
    {
        ...
        {
            ptr->leftchild->depth=ptr->depth+1;
            ptr->leftchild->offset=ptr->offset-pow(max_depth-ptr->leftchild->depth,2);//offset formula not right
          //  ptr->leftchild->offset=ptr->offset-(70/(pow(2,max_depth-ptr->depth)+1));
        ...
        {
            ptr->rightchild->depth=ptr->depth+1;
         ptr->rightchild->offset=ptr->offset+pow(max_depth-ptr->rightchild->height,2);
      // ptr->leftchild->offset=ptr->offset+(70/(pow(2,max_depth-ptr->depth)+1));
